...
BEGIN_DATADESC( CHL2_Player )
	...
	*/
	...
	DEFINE_FIELD( m_ArmorState , FIELD_INTEGER),
...
bool CHL2_Player::ApplyKevlar(int Armor, float powerMultiplier)
{
	const float MAX_NORMAL_BATTERY = 200;
	if ((m_nArmorStateNumber == ARMOR_NON) || (m_nArmorStateNumber == ARMOR_BOTH) || (m_nArmorStateNumber == ARMOR_HELMET)){
		if ((ArmorValue() >= 100) && (ArmorValue() < MAX_NORMAL_BATTERY) && IsSuitEquipped())
		{
			int pct;
			char szcharge[64];
			IncrementArmorValue(60 * powerMultiplier, MAX_NORMAL_BATTERY);
			CPASAttenuationFilter filter(this, "ItemBattery.Touch");
			EmitSound(filter, entindex(), "ItemBattery.Touch");
			...
			Q_snprintf(szcharge, sizeof(szcharge), "!HEV_%1dP", pct);
			if (m_nArmorStateNumber == ARMOR_HELMET || (m_nArmorStateNumber == ARMOR_BOTH)) { (m_nArmorStateNumber == ARMOR_BOTH); }
			else
			(m_nArmorStateNumber == ARMOR_KEVLAR);
			//UTIL_EmitSoundSuit(edict(), szcharge);
			//SetSuitUpdate(szcharge, FALSE, SUIT_NEXT_IN_30SEC);
			if (ArmorValue())
				return true;
...
bool CHL2_Player::ApplyHelmet(float powerMultiplier)
{
		const float MAX_NORMAL_BATTERY = 200;
		if ((m_nArmorStateNumber == ARMOR_NON) || (m_nArmorStateNumber == ARMOR_BOTH) || (m_nArmorStateNumber == ARMOR_KEVLAR)){
			if ((ArmorValue() >= 100) && (ArmorValue() < MAX_NORMAL_BATTERY) && IsSuitEquipped())
			{
				int pct;
				char szcharge[64];
				IncrementArmorValue(40 * powerMultiplier, MAX_NORMAL_BATTERY);
				CPASAttenuationFilter filter(this, "ItemBattery.Touch");
				EmitSound(filter, entindex(), "ItemBattery.Touch");
				...
				Q_snprintf(szcharge, sizeof(szcharge), "!HEV_%1dP", pct);
				if (m_nArmorStateNumber == ARMOR_KEVLAR || (m_nArmorStateNumber == ARMOR_BOTH)) (m_nArmorStateNumber == ARMOR_BOTH); 
				else
				(m_nArmorStateNumber == ARMOR_HELMET);
				//UTIL_EmitSoundSuit(edict(), szcharge);
				//SetSuitUpdate(szcharge, FALSE, SUIT_NEXT_IN_30SEC);
				if (ArmorValue())
					return true;
